<template>
  <footer
    class="text-gray-500 bg-black px-4 py-5 mb-8 rounded-[30px] max-w-screen-xl mx-auto md:px-8"
  >
    <div class="gap-6 justify-between md:flex p-10">
      <div class="flex-1">
        <div class="max-w-sm">
          <img src="../assets/logo.png" class="w-32" />
          <p class="leading-relaxed mt-2 text-gray-100 text-[15px]">
            We empower caregivers by supporting their mental health journey.
            Navigate your mental health with us.
          </p>
        </div>
      </div>
      <div
      ...
      >
        <ul v-for="(item, index) in footerNavs" :key="index" class="space-y-4">
          <h4 class="text-gray-100 font-medium">{{ item.label }}</h4>
          <li v-for="(el, idx) in item.items" :key="idx">
            <a class="hover:underline text-gray-300">
              {{ el.name }}
            </a>
          </li>
        </ul>
    ...
    <div class="mt-8 py-6 border-t items-center justify-between sm:flex">
      <div class="mt-4 sm:mt-0 text-gray-300">
        &copy; 2024 Stawi Global All rights reserved.
      </div>
        ...
        <ul class="flex items-center space-x-4">
          ...
          >
            <a href="javascript:void()">
              <svg class="svg-icon w-6 h-6 text-gray-100" viewBox="0 0 20 20">
                <path
                  fill="none"
